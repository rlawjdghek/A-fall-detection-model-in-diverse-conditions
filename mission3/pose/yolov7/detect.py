...
# 여기서 추가로 색깔까지 구별하는 알고리즘 반환해야함.
def pose_estimate(model, img, im0s, opt):
    pred = model(img, augment=False)[0]
    pred = non_max_suppression(pred, opt.m3_pose_conf_thres, opt.m3_pose_iou_thres, classes=opt.m3_pose_classes, agnostic=opt.m3_pose_agnostic_nms, kpt_label=True)
    bbox = []
    outputs = []
    w_outputs = []
    # Process detections
    for i, det in enumerate(pred):  # detections per image
        ...
        if len(det):
            ...
            for det_index, (*xyxy, conf, cls) in enumerate(reversed(det[:,:6])):
                coords = [
                    [int(det[det_index][0]), int(det[det_index][1])],
                    [int(det[det_index][2]), int(det[det_index][1])],
                    [int(det[det_index][2]), int(det[det_index][3])],
                    [int(det[det_index][0]), int(det[det_index][3])]
                ]
                bbox.append(coords)
                #xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                #for k in range(6,len(det[det_index]), 3):
                #    print(k, det[det_index][k:k+2])
                #exit()
                w_x1, w_y1, w_x2, w_y2 = det[det_index][39], det[det_index][40], det[det_index][42], det[det_index][43]
                x1, y1, x2, y2 = det[det_index][33], det[det_index][34], det[det_index][36], det[det_index][37]
                if y1 <= y2:
                    outputs.append([float(x1), float(y1)])
                else:
                    outputs.append([float(x2), float(y2)])
                w_outputs.append([w_x1, w_y1, w_x2, w_y2])
    return bbox, outputs, w_outputs
            ...
            if len(det):
                ...
                for det_index, (*xyxy, conf, cls) in enumerate(reversed(det[:,:6])):
                    if save_txt:  # Write to file
                        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                        save_kpts = det[det_index][33:39]
                        line = (cls, *xywh, conf) if opt.save_conf else (cls, *xywh, *save_kpts)  # label format
