...
def recognizer_predict(model, converter, test_loader, batch_max_length,\
    ...
    result = []
    with torch.no_grad():
        for i, image_tensors in enumerate(test_loader):
            batch_size = image_tensors.size(0)
            image = image_tensors.to(device)
            # For max length prediction
            length_for_pred = torch.IntTensor([batch_max_length] * batch_size).to(device)
            text_for_pred = torch.LongTensor(batch_size, batch_max_length + 1).fill_(0).to(device)
            preds = model(image, text_for_pred)  ######## real recognition prediction
...
def get_recognizer(recog_network, network_params, character,\
    ...
    if recog_network == 'generation1':
        model_pkg = importlib.import_module("OCR.ref.EasyOCR.easyocr.model.model")
        model = model_pkg.Model(num_class=num_class, **network_params)
    elif recog_network == 'generation2':
        model_pkg = importlib.import_module("OCR.ref.EasyOCR.easyocr.model.vgg_model")
        model = model_pkg.Model(num_class=num_class, **network_params)
    else:
        from .model.taemo_recog import Model
